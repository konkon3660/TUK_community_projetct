 ...
 * 대분류(게시판/공지사항/민원/채팅/추천)와 로그아웃만 두는 허브. 게시판 세부(자유/공동구매/
 * 학과/기숙사)는 BoardMenuPanel("boardMenu")로 한 단계 내려갔다. 서버 통신은 없고 순수
 * 네비게이션만 담당한다. 다른 화면으로 데이터와 함께 전환할 때는 mainFrame.getScreen("이름")을
 * 캐스팅해서 open(...)을 먼저 호출한 뒤 mainFrame.switchTo("이름")을 부르는 컨벤션을 따른다
 * (documents/06_gui.md 참고).
 */
public class HomePanel extends JPanel {
    private final MainFrame mainFrame;
    private final JButton boardMenuButton = new JButton("게시판");
    ...
    public HomePanel(MainFrame mainFrame) {
        ...
        boardMenuButton.addActionListener(e -> mainFrame.switchTo("boardMenu"));
    ...
    private void initLayout() {
        ...
        // 대분류 5개를 세로 한 줄로, 맨 아래 로그아웃 (세부 게시판은 "게시판" 안의 boardMenu 화면).
        c.gridwidth = 2;
        c.insets = new Insets(4, 4, 4, 4);
        c.fill = GridBagConstraints.HORIZONTAL;
        JButton[] buttons = {
                boardMenuButton, noticeBoardButton, complaintButton,
    ...
    private void openNoticeBoard() {
        // 공지는 홈 대분류에서 바로 연다 (게시판 하위가 아니라). 뒤로가기는 홈으로.
        ((PostListPanel) mainFrame.getScreen("postList")).open(BoardKey.NOTICE, "home");
