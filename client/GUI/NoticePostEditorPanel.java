...
public class NoticePostEditorPanel extends JPanel {
    ...
    private void save() {
        ...
        // (덮어쓴 것처럼) 보인다. 그래서 수정 시에도 새 객체를 만들어 보낸다. 대상 범위는 수정
        // 화면에서 바꿀 수 없으므로(위 setEnabled(false)) 픽커 값이 아니라 기존 값을 그대로 쓴다.
        NoticePost post = isNew
                ...
                : new NoticePost(editingPost.getId(), titleField.getText(), editingPost.getAuthorId(),
                        contentArea.getText(), attachmentPicker.getFilePath(), attachmentPicker.getImagePath(),
                        editingPost.getCreatedAt(), editingPost.getTargetDepartments(), editingPost.isDormNotice());
